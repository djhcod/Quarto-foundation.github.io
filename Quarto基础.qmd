---
title: Quarto基础
subtitle: 面向多编程语言的下一代Markdown编辑器
author: 杜俊宏
date: 2023/11/10
date-modified: now
...
reference-location: margin # 脚注的默认展示位置。默认为文档最后。
citation-location: margin # 参考文献的默认展示位置。默认为文档最后。 
format: 
  html:
    theme: 
      light: flatly # 亮色主题。常用主题：Cosmo、Flatly和Darkly
      dark: darkly # 暗色主题。
    embed-resources: true # 是否将所有源文件嵌入HTML文件
    code-tools: true
...
Quarto是一个支持多种编程语言的新一代R Markdown，拥有数十个新的特性和功能，同时能够兼容和渲染大多数现有的RMD文件，而无需额外修改。Quarto主要由三大部分组成：metadata, text, 和 code。其中metadata是文档开头写在"\-\--"之间的句子，它采用的是[YAML语法](https://en.wikipedia.org/wiki/YAML)，所以有时候也别叫做YAML metadata 或者 the YAML frontmatter。YAML定义了Quarto的性质，比如title、author、date、指定output文件类型等。在YAML metadata中采用正确的缩进十分重要。
...
![](images/截屏2023-11-09%2009.39.13-01-01.png){width="244"}
...
可用的值参见 @tbl-设置图表的标题 。这份文档的参考文献就设置为了在页面右侧（margin）展示。
